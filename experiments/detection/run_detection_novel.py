...
Detectores:
  --detector yolo : Ultralytics YOLO (requiere torch; correr en PC/Colab con GPU/CPU).
  --detector hog  : HOG-SVM de personas (OpenCV, offline; SOLO referencial, no concluyente).

Uso (en tu PC, con el venv y ultralytics instalados):
  python experiments/detection/run_detection_novel.py --detector yolo --weights yolov8n.pt

Salida: experiments/results/metrics_reports/detection_novel.csv  (+ resumen por método)
"""
...
OUT = ROOT / "experiments" / "results" / "metrics_reports" / "detection_novel.csv"
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--detector",choices=["hog","yolo"],default="hog")
    ap.add_argument("--weights",default="yolov8n.pt"); ap.add_argument("--limit",type=int,default=0)
    a=ap.parse_args()
    det = HOG() if a.detector=="hog" else YOLOv(a.weights)
    ...
    df=pd.DataFrame(rows); OUT.parent.mkdir(parents=True,exist_ok=True)
